...
function Remove-SOCatStore {
	...
	Process {
        ...
        if (Test-IP -IP $($SOConnections.Server)) {
            if (!(Get-SOCatStores | where {$_.Name -eq $SOCatStoreName -and $_.System -eq $($SOConnections.Server)})) {throw "Store $SOCatStoreName does not exist."}
            $StoreCall = @{uri = "https://$($SOConnections.Server)/storeonceservices/cluster/servicesets/$SSID/services/cat/stores/$((Get-SOCatStores | where {$_.Name -eq $SOCatStoreName -and $_.System -eq $($SOConnections.Server)}).ID)";
                            Method = 'DELETE';
